...
def train_rl_agent(data_path, total_timesteps=200000, eval_freq=10000, n_envs=8, train_test_split=0.8):
    ...
    # Стабилизированные параметры PPO для прибыльной торговли
    model = PPO(
        "MlpPolicy",
        env,
        learning_rate=1e-4,  # Увеличен для лучшей сходимости (был 3e-5)
        ...
        ent_coef=0.02,  # Увеличено для поощрения исследования (было 0.01)
    ...
    def make_eval_env():
        ...
        env = TradingEnvironment(eval_df, debug=False, episode_length=100)  # Уменьшаем длину эпизода для оценки
